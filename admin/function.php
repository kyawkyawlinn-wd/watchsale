...
function addproduct()
{
    ...
    elseif($productname!="" && $photo!="")
    {
        ...
        {
            $query = "SELECT catname FROM brand WHERE catid = '$categoryid'";
            $cat_query = mysqli_query($conn, $query);
            $category = mysqli_fetch_assoc($cat_query)['catname'];
            $query="insert into product(productname,catid,catname,price,qty,photo) values('$productname','$categoryid','$category','$price','$quantity','$photo')";
            $go_query=mysqli_query($conn,$query);
            if(!$go_query)
            {
                die("QUERY FAILED".mysqli_error($conn));
